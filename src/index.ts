...
// import { CurlImpersonateResponse } from "./interfaces";
...
// export type BinaryName = `${string}-${NodeJS.Architecture}${string}`;

// export type Browser = "chrome" | "firefox" | "safari";

// export const BINARY_PATH = path.join(__dirname, "..", "bin", process.platform);

// export const getPlatformBinary = async (browser: Browser | undefined = "chrome"): Promise<string> => {
//     const VALID_BINARIES: Record<string, BinaryName[] | undefined> = {
//         win32: [
//             "chrome-x64.exe",
//         ],
//         darwin: [
...
//         ]
//     }
//     const resolution = VALID_BINARIES[process.platform];
//     if (resolution === undefined) {
//         throw new Error(`Unable to find valid binary resolution for ${process.platform}`);
//     }
//     if (process.platform === "win32") {
//         return resolution[0];
//     }
//     const executableName = `${browser}-${process.arch}`;
//     const binary = resolution.find(res => res.startsWith(executableName));
//     if (binary === undefined) {
//         throw new Error(`Unable to find executable '${executableName}'`);
//     }
//     return binary;
// }
...
// export const buildHeaders = (headers: Record<string, string>): string => {
//     return Object.entries(headers).map(([key, value]) => `-H "${key}: ${value}"`).join(" ");
// }

// export const doRequest = async (url: string, options: CurlImpersonateRequestOptions = {}): Promise<CurlImpersonateResponse> => {
//     const {
//         browser,
//         customFlags = [],
//         customHeaders = {},
//         body,
//         method = "GET",
//         presetName = "chrome-110"
//     } = options;

//     const binary = await getPlatformBinary(browser);
...
//         `-X ${method}`
//     ];
//     const headers = buildHeaders({ ...customHeaders, ...preset.headers });
//     const cwd = BINARY_PATH;

//     const binpath = path.join(BINARY_PATH, binary);
//     const args = [
//         ...flags,
//         headers,
//         `"${url}"`,
//     ].join(" ");
//     const command = `${binpath} ${args}`;
